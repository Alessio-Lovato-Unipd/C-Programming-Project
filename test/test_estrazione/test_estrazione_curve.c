...
void verifica_ricerca_nome_turbina_e_alcuni_valori(void)
{
	...
	TEST_ASSERT_EQUAL_FLOAT(0.0, elemento_cercato->power_curves[0]);
	TEST_ASSERT_EQUAL_FLOAT(115000, elemento_cercato->power_curves[7]);
...
void verifica_valori_su_curve_diverse(void)
{
	...
	cerca_curva_potenza = cerca_dati_turbina("GE100/2500", 0.0, head);
	TEST_ASSERT_EQUAL_STRING("ENO114/3500", cerca_coefficienti->nome);
	TEST_ASSERT_EQUAL_STRING("GE100/2500", cerca_curva_potenza->nome);
	
	TEST_ASSERT_EQUAL_FLOAT(-1, cerca_coefficienti->power_coefficients[0]);
	TEST_ASSERT_EQUAL_FLOAT(-1, cerca_coefficienti->power_coefficients[11]);
	TEST_ASSERT_EQUAL_FLOAT(0.426, cerca_coefficienti->power_coefficients[12]);
