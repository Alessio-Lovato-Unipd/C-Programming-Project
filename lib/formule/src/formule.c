...
float barometrico(float h_dati, float p1, float T_mozzo, float h_mozzo, int *errore)//T_x (mozzo) la passo per paramtri perchè non posso sapere quale metodo di calcolo va usato
{
    if (h_dati < 0) {
        errore[ERR_H_DATI] = 1;
        return -1;
